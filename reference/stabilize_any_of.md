...
[`to()`](https://stbl.wrangle.zone/reference/to.md): it tries to coerce
`x` to each type given in `...` (as a prototype such as
...
- ...:
  ...
  etc.), `to_*` functions
  ([`to_chr()`](https://stbl.wrangle.zone/reference/to_chr.md), etc.),
  functions produced by `specify_*()` calls
  ...
  that return their input unchanged. For `to_any_of()`: prototype
