...
- All `stbl_*_to_*` C callables now return a named list
  `list(result = <type>, valid = <lgl>)`, matching the existing `ffi_*`
  internal design. The `ffi_*` internal symbols have been removed. The
  `lst_to_*` callables are also standardized to this pattern, returning
  per-element validity flags rather than `NULL` on fast-path failure. C
  consumers can now unpack a consistent shape and inspect validity
  without a second-pass `stbl_*_are_*ish` call. R-side `to_*()`
