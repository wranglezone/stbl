...
* New function `stabilize_all_of()` validates `x` by applying each unnamed stabilizer function in `...` to the original `x`, independently; `x` must satisfy every spec, and every spec must produce the same coerced value, or an informative error is thrown. `stabilise_all_of()` is a synonym (#278).
