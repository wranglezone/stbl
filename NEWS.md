...
* `are_fct_ish()` and `is_fct_ish()` now accept a `max_levels` argument to limit the number of unique non-`NA` values allowed across the full vector after applying `to_na` (#noissue).
