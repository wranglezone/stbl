...
* New `stabilize_df()` and `specify_df()` to validate data frame structure and contents, including column-level validators, row count constraints (`.min_rows`, `.max_rows`), and required column name checks (`.col_names`) (#199). `to_df()` coerces compatible inputs such as named lists to data frames.
