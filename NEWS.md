...
* New function `assert_not()` errors when `x` would be accepted (coerced or validated) by a single `spec`, and returns `x` unchanged otherwise; it composes with the `stabilize_*_of()` family to require that a value is *not* something (#289).
