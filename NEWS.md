...
* New function `stabilize_one_of()` validates `x` against every stabilizer function in `...` and requires that exactly one succeeds, returning that function's result; an error is thrown if zero functions succeed (combining the individual failure messages) or if more than one succeeds (naming the specifications that matched). This mirrors JSON Schema `oneOf` semantics and is intended for mutually exclusive specifications. New function `to_one_of()` works analogously with type prototypes via `to()` (#286).
