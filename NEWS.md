...
* `to_chr()`, `to_dbl()`, `to_fct()`, `to_int()`, and `to_lgl()` list methods now use C routines for coercion, improving performance. Singly-nested lists (e.g., `list(list("a"), "b")`) are now flattened correctly. Non-character scalar types (e.g., doubles, integers, logicals) in `to_chr()` are now correctly converted to strings rather than being rejected (#239).
