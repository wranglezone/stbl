...
* New function `assert_contains()` counts how many elements of `x` match a single `spec` and errors unless that count is between `min_matches` and `max_matches`, returning `x` unchanged otherwise (#290).
