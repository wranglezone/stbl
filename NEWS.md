...
* New `to()` is a fast drop-in replacement for `vctrs::vec_cast()`. It coerces `x` to the type of `.to` by dispatching on the class of `.to` and calling the appropriate `to_*()` function. Logical, integer, double, and character targets use a C implementation (`stbl_to()`) for maximum speed; factor and list targets delegate to `to_fct()` and `to_lst()`. `stbl_to()` is also registered as a C callable in the public C API for use by packages such as tibblify (#182).
