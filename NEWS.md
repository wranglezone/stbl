...
* New function `assert_contains()` counts how many elements of `x` match a single `spec` and errors unless that count is between `min_matches` (which can be `0`, to check only an upper bound) and `max_matches`, returning `x` unchanged otherwise (#290).
