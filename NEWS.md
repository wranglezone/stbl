...
* New function `assert_contains()` counts how many elements of `x` match a `spec` and checks that the count is within a specified range (#290).
...
* New functions `specify_all_of()`, `specify_any_of()`, and `specify_one_of()` create pre-configured `stbl_specified_fn`s that call `stabilize_all_of()`, `stabilize_any_of()`, and `stabilize_one_of()` respectively, letting composition specs be reused and nested inside other `specify_*()`/`stabilize_*()` calls (#288).
* New function `stabilize_all_of()` validates `x` by applying each stabilizer function in `...`, failing if any stabilizer fails or if the stabilized values are different (#278).
* New function `stabilize_each()` applies a single `spec` to every element of `x`. New function `to_each()` works analogously but stops at the first failing element. New function `specify_each()` creates a pre-configured `stbl_specified_fn` that calls `stabilize_each()` with a fixed `spec`, letting an element-wise spec be reused and nested inside other `specify_*()`/`stabilize_*()` calls (#287).
* New function `stabilize_any_of()` validates `x` by trying each stabilizer function in `...` in order, returning the first successful result. If all functions fail, an informative error combining the individual failure messages is thrown. New function `to_any_of()` works analogously, coercing to the first successful type prototype in `...` via `to()` (#215, #285).
* New function `stabilize_one_of()` validates `x` against every stabilizer function in `...` and requires that exactly one succeeds, returning that function's result; an error is thrown if zero functions succeed or if more than one succeeds. New function `to_one_of()` works analogously with type prototypes via `to()` (#286).
...
* `stabilize_int()`, `stabilize_int_scalar()`, `stabilize_dbl()`, `stabilize_dbl_scalar()`, `specify_int()`, `specify_int_scalar()`, `specify_dbl()`, and `specify_dbl_scalar()` gain new `exclusive_min_value` and `exclusive_max_value` arguments, enforcing strict (open-interval) numeric bounds that reject values equal to the boundary, matching JSON Schema `exclusiveMinimum`/`exclusiveMaximum` semantics (#276).
* `stabilize_int()`, `stabilize_int_scalar()`, `stabilize_dbl()`, `stabilize_dbl_scalar()`, `specify_int()`, `specify_int_scalar()`, `specify_dbl()`, and `specify_dbl_scalar()` gain a new `multiple_of` argument, requiring that values be a multiple of the given value; `stabilize_dbl()`/`stabilize_dbl_scalar()` apply a small relative tolerance to avoid floating-point false negatives (#283).
...
* New function `locale_datetime_formats()` returns a locale-aware set of `strptime()`-style date/date-time formats. `stabilize_date()`, `stabilize_date_scalar()`, `stabilize_dttm()`, `stabilize_dttm_scalar()`, and their `specify_*()` factories gain a new `accepted_datetime_formats` argument (defaulting to `locale_datetime_formats()`) so that locale-dependent formats such as `"11/13/2018"` are accepted where unambiguous; `to_date()` and `to_dttm()` are unaffected and continue to require the strict RFC 3339 shape (#326).
