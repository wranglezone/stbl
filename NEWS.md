...
* `are_dbl_ish()`, `are_int_ish()`, `are_lgl_ish()`, `to_dbl()`, `to_int()`, and `to_lgl()` for character input are now implemented in C. Benchmarks show a significant speedup (about 5-20x) for large character vectors (#217, #218, #219, #221). `are_int_ish()` and `to_int()` for double input are also now implemented in C.
